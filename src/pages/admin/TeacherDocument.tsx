...
type RenderItem = SentDoc | BroadcastItem;
...
const TeacherStudentDocuments: React.FC<Props> = ({ studentId: studentIdProp }) => {
    ...
    useEffect(() => {
        ...
        const t = setTimeout(async () => {
            ...
            try {
                const res = await adminStudentAPI.getAll({ page: 1, limit: 20, search: q });
    ...
    const listForRender: RenderItem[] = useMemo(() => {
        if (effectiveStudentId) return (list || []) as SentDoc[];

        const groups = new Map<string, SentDoc[]>();
        (list || []).forEach((d) => {
            const key = [
                d.fileUrl || "",
                d.name || "",
        ...
        groups.forEach((arr, key) => {
            const uniqueRecipients = new Set(
                arr.map(a => a.userId?._id).filter(Boolean) as string[]
            );
            if (uniqueRecipients.size > 1) {
                ...
                out.push({
                    ...
                    recipientsCount: uniqueRecipients.size,
                });
            } else {
                out.push(arr[0]);
            }
        ...
        out.sort((a, b) => {
            const ta = ("uploadDate" in a && a.uploadDate) ? new Date(a.uploadDate!).getTime() : 0;
            const tb = ("uploadDate" in b && b.uploadDate) ? new Date(b.uploadDate!).getTime() : 0;
            return tb - ta;
        });

        return out;
    }, [list, effectiveStudentId]);
    // --- upload ---
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            {isLoading ? (
                <p>Đang tải...</p>
            ) : (listForRender?.length ?? 0) === 0 ? (
                <p className="text-gray-500 italic">
                    {effectiveStudentId ? "Chưa có tài liệu đã gửi cho học viên này." : "Chưa có tài liệu đã gửi."}
                </p>
            ) : (
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {listForRender.map((doc) => {
                        const isBroadcast = (doc as any).isBroadcast === true;
                        const fileUrl = doc.fileUrl;
                        const name = doc.name;
                        const type = doc.type;
                        const uploadDate = (doc as any).uploadDate;
                        const status = doc.status;
                        const statusText = doc.statusText;
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            {isBroadcast ? (
                                                <p className="text-xs text-muted-foreground">
                                                    Người nhận: <span className="font-semibold">Tất cả học viên</span>
                                                    {(doc as any).recipientsCount ? ` (${(doc as any).recipientsCount})` : ""}
                                                </p>
                                            ) : (
                                                <p className="text-xs text-muted-foreground">
                                                    Người nhận: <span className="font-medium">{(doc as SentDoc).userId?.name || "Không rõ"}</span>
                                                    {(doc as SentDoc).userId?.email ? ` (${(doc as SentDoc).userId!.email})` : ""}
                                                </p>
                                            )}

                                            <p className="text-xs text-muted-foreground">
                                                Người gửi: <span className="font-medium">{uploadedBy?.name || "Bạn"}</span>
                                                {uploadedBy?.email ? ` (${uploadedBy.email})` : ""}
                                            </p>

                                            <div className="flex items-center justify-between text-xs text-muted-foreground mt-1">
                                                <span>{type}</span>
                                                <span>{uploadDate ? new Date(uploadDate).toLocaleDateString() : ""}</span>
                                            </div>

                                            <p className="text-xs mt-1">
                                                Trạng thái:{" "}
                                                <span
                                                    className={`font-semibold ${status === "approved" ? "text-green-600"
